...
plot_GSEA <- function(geneset, geneset_name, contrast){
	...
	breaks_ES = c(signif(min(merged_subset_m$ES_blood), 1)-0.05, signif(max(merged_subset_m$ES_blood), 1)+0.05)
	limits_ES = c(signif(min(merged_subset_m$ES_blood), 1)-0.05, signif(max(merged_subset_m$ES_blood), 1)+0.05)
	labels_ES = c(signif(min(merged_subset_m$ES_blood), 1)-0.05, signif(max(merged_subset_m$ES_blood), 1)+0.05)

	## color dots that are not significant gray
	merged_subset_m$value_ES_withNA <- ifelse(merged_subset_m$padj_blood < 0.10, merged_subset_m$ES_blood, merged_subset_m$ES_blood == NA)
	merged_subset_m$CT <- paste0("blood")

	plot <- ggplot(merged_subset_m, aes(x = CT, y = pathway, fill = value_ES_withNA, size = -log10(padj_blood), color = CT)) + 
			geom_point(shape = 21, stroke = 0.8) +
			scale_color_manual(values = rep("black", 7), name = NULL, guide = FALSE) +
	   		scale_size(name = "-log10(padj)") +
	   		scale_fill_gradientn(colours = rev(brewer.pal(11,"RdYlBu")), guide = guide_colorbar(frame.colour = "black", ticks.colour = "black", title = "enrichment score", frame.linewidth = 1, ticks = FALSE), breaks = breaks_ES, limits = limits_ES, labels = labels_ES, na.value = "grey75") +
			guides(colour = FALSE) + 
