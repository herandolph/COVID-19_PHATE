...
## COVID severity genes
posBeta <- readRDS("inputs/genelists/COVID_severity.rds")
posBeta <- list(Reduce(union, posBeta))
## 374 genes
names(posBeta) <- "pos_beta"
...
scores <- gsva(as.matrix(exp_gvsa),
				gset.idx.list = posBeta,
				method = "ssgsea",
				verbose = FALSE,
				parallel.sz = 1)

scores <- as.data.frame(t(scores))
colnames(scores) <- paste0("COVID_severity_score")
scores$Assigned_Name <- rownames(scores)
scores <- scores[,c(2,1)]

## write combined scores
write.table(scores, paste0(results_dir,"GSVA_COVID_severity_score.txt"), sep = ",", row.names = FALSE, quote = FALSE)
...
plot <- ggplot(df, aes(x = PHATE_cluster, y = COVID_severity_score, color = PHATE_cluster, fill = PHATE_cluster)) +
			...
			#ylim(min(df$IFN_score), max(df$IFN_score)) +
			...
			xlab("PHATE cluster") +
			ylab("ssGSEA COVID severity score, DSO 11") +
			ggtitle(paste0("bulk whole blood")) 

pdf(paste0(results_dir,"by_cluster_COVID_severity_score.pdf"), width = 4, height = 3.5)
print(plot)
dev.off()
